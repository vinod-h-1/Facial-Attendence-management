...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'vinodh.mca20@rvce.edu.in' ")
...
window.title("RV college of engineering- MCA DEPARTMENT Attendance System")
window.configure(background='#D9D2D1')

im=ImageTk.PhotoImage(file='photo.png')
label=Label(window)
label.place(x=0,y=0)
label.config(image=im)

frame1 = tk.Frame(window, bg="#c79cff")
frame1.place(relx=0.11, rely=0.27, relwidth=0.39, relheight=0.65)

frame2 = tk.Frame(window, bg="#c79cff")
frame2.place(relx=0.51, rely=0.27, relwidth=0.38, relheight=0.65)



message3 = tk.Label(window, text="Face Recognition Based Attendance Monitoring System" ,fg="black",bg="#D9D2D1" ,width=67 ,height=1,font=('comic', 29, ' bold '))
message3.place(x=50, y=149)


frame3 = tk.Frame(window, bg="#D9D2D1")
frame3.place(relx=0.55, rely=0.22)

frame4 = tk.Frame(window, bg="#D9D2D1")
frame4.place(relx=0.13, rely=0.22)

datef = tk.Label(frame4, text = day+"-"+mont[month]+"-"+year, fg="#ff61e5",bg="#D9D2D1" ,width=33 ,height=1,font=('comic', 22, ' bold '))
datef.pack(fill='both',expand=1)

clock = tk.Label(frame3,fg="#ff61e5",bg="#D9D2D1" ,width=23 ,height=1,font=('comic', 22, ' bold '))
...
menubar.add_command(label='Change Password', command = change_pass)
menubar.add_command(label='Contact Us', command = contact)
menubar.add_command(label='Exit',command = window.destroy)
menubar.add_cascade(label='                                  RV COLLEGE OF ENGINEERING',font=('comic', 29, ' bold '))
